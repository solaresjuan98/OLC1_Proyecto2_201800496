...
class TreeTraversal {
    ...
    traverse_gv(node) {
        ...
        if (node.value.includes("\"")) {
            ...
            if (node.type != "NON_TERMINAL") {
                ...
                if (node.value === 'int' || node.value === 'double'
                    || node.value === 'char' || node.value === 'boolean') {

                    translate += this.GenTab(tab) + "var ";
                }
                else if(node.value === 'String' && node.value === 'String_'){

                    //
                }
                else if(node.value === 'String' && node.type === 'String'){
                    translate += this.GenTab(tab) + "var ";
                }
                ...
                else if (node.value === ';' && node.type === ';_') {
                ...
                }else if(node.type === 'text_string_sq'){
                    translate += node.value;
                }
                else if(node.type === 'text_string_qm'){
                    translate += node.value;
                }
                ...
                // ----- NOT TRANSLATED
                    ...
                    || node.type === 'String_' || node.value === 'System' || node.value === 'out') {
                    //
                }
                else {
                    translate += node.value;
    ...
    translate() {
        ...
        fs.writeFileSync('result/translation.js', translate, 'utf-8');
    }
